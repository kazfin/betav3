...
if (PersonRoles[i] === "Mother") {
    age = getRandomIntInclusive(16, 45);
} else if (PersonRoles[i] === "Father") {
    age = getRandomIntInclusive(16, 70);
} else if (PersonRoles[i] === "Brother" || PersonRoles[i] === "Sister") {
    // Parent must exist and be processed already
    const parent = PersonRoles.includes("Mother") ? 
        PersonAges[PersonRoles.indexOf("Mother")] : 
        PersonAges[PersonRoles.indexOf("Father")];
    age = getRandomIntInclusive(0, parent - 16);
} else if (PersonRoles[i] === "Grandmother" || PersonRoles[i] === "Grandfather") {
    // Grandparent age: 25–45 years older than parent
    const parent = PersonRoles.includes("Mother") ? 
        PersonAges[PersonRoles.indexOf("Mother")] : 
        PersonAges[PersonRoles.indexOf("Father")];
    age = getRandomIntInclusive(parent + 25, 80);
} else if (PersonRoles[i] === "Aunt" || PersonRoles[i] === "Uncle") {
    // Aunt/Uncle age: ±15 years from grandparent's age
    const grandparent = PersonRoles.find(r => r === "Grandmother" || r === "Grandfather");
    if (grandparent) {
        const grandparentAge = PersonAges[PersonRoles.indexOf(grandparent)];
        age = getRandomIntInclusive(grandparentAge - 15, grandparentAge + 15);
    } else {
        age = getRandomIntInclusive(30, 60); // Fallback
    }
} else if (PersonRoles[i] === "Cousin") {
    // Cousin age: 0–(Aunt/Uncle's age - 16)
    const auntUncle = PersonRoles.find(r => r === "Aunt" || r === "Uncle");
    if (auntUncle) {
        const auntUncleAge = PersonAges[PersonRoles.indexOf(auntUncle)];
        age = getRandomIntInclusive(0, auntUncleAge - 16);
    } else {
        age = getRandomIntInclusive(0, 18); // Fallback
    }
}

// Clamp age to 0–80 and push
PersonAges.push(Math.min(80, Math.max(0, age)));
}
}
